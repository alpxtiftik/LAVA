...
> **Platform:** LAVA runs on **Linux only** (tested on Kali / Ubuntu). EMBA and the
> firmware-extraction tooling it relies on need a native Linux kernel.
...
- **Ollama:** Installed and running locally — `curl -fsSL https://ollama.com/install.sh | sh`, then `ollama serve`.
- **Local AI Model:** Pulled via Ollama — e.g. `ollama pull qwen2.5-coder:7b` (or `llama3`).
- **EMBA** (only for the end-to-end "Full Pipeline" firmware scanning): installed natively.
...
2. Update your configuration in `config/ai_config.env`:
   ...
   # EMBA executable (for the end-to-end pipeline)
...
> **Run scans as your normal user, not root.** EMBA needs `sudo` (the scripts
> handle that internally), but the `claude` / `agy` CLIs and their login state
> live in your user account — running the whole pipeline with `sudo` hides them.
> If you launch the full pipeline as root anyway, LAVA now drops back to
> `$SUDO_USER` for the AI step automatically.
...
Then use LAVA exactly as before (GUI *Start Scan*, `run_lava.sh`,
...
No compilation needed. Simply run the startup script, which will automatically
configure a virtual environment, install dependencies, and launch the UI:
...
1. You will see a Mode Toggle at the top:
   - **Analyze EMBA Log Directory:** Provide the path to a completed EMBA log directory. LAVA will run AI analysis on it.
   - **Full Pipeline (Scan Firmware):** Select a `.bin` or `.tar` firmware file. LAVA will automatically launch EMBA to scan the firmware and immediately begin AI analysis on the results.
...
**To analyze an existing EMBA log directory:**
```bash
...
python3 src/core/parser.py --log-dir "emba_iotgoat_log" --out findings.json --merged-out merged_findings.json

# 2. Enrich findings with actual file context
python3 src/core/enricher.py --merged merged_findings.json --log-dir "emba_iotgoat_log" --out enriched_findings.json
